  ...
  <div class="row">
      <div class="col-sm-12">
          <div class="page-title-box">
              ...
              <h4 class="page-title">Data Transaksi rental</h4>
          </div>
      </div>
  </div>
        ...
        <div class="card m-b-30">
          ...
          <div class="table-responsive">
            ...
            <table id="datatable" class="table table-bordered">
              ...
              $query = "SELECT * FROM `tb_rental` INNER JOIN `tb_pelanggan` ON `tb_rental`.`pelangganid` = `tb_pelanggan`.`pelangganid` INNER JOIN `tb_users` ON `tb_users`.`userid` = `tb_rental`.`userid` INNER JOIN `tb_jenis` ON `tb_jenis`.`kd_jenis` = `tb_rental`.`kd_jenis` ORDER BY `tb_rental`.`id_rental` DESC";
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <?php if($row['status_pengambilan'] == 0) { ?>
                      <a href="?page=rental&aksi=diambil&id=<?= $row['id_rental']; ?>" class="btn btn-warning <?= ($row['status_pembayaran'] == 0) ? 'disabled' : ''; ?>" onclick="return confirm('Apakah anda yakin?');">Belum Diambil</i></a>
                  ...
                  <td>
                    ...
                    <?php if($row['status_pembayaran'] == 1) { ?>
                      ...
                      <a href="page/cetak_transaksi.php?id=<?= $row['id_rental']; ?>" class="btn btn-success" target="_blank"><i class="fa fa-download"> Cetak</i></></a>

                    <!-- jika status pembayaran = 0 -->
                    <?php }elseif($row['status_pembayaran'] == 0){ ?>

                      <a href="?page=rental&aksi=detail&id=<?= $row['id_rental']; ?>" class="btn btn-primary mb-2"><i class="fa fa-eye"></i> Detail</a>
                      <a href="?page=rental&aksi=lunasi&id=<?= $row['id_rental']; ?>" class="btn btn-success mb-2" onclick="return confirm('Apakah anda yakin transaksi rental sudah lunas ?');"><i class="fa fa-money"></i> Lunasi</a>
